...
export default function DashboardLayout({ children }) {
  ...
  const shellValue = useMemo(() => ({ waStatus }), [waStatus]);
  ...
  const activeLabel = navItems.find((item) => pathname === item.href || pathname.startsWith(`${item.href}/`))?.label || 'Auto Reply';
      ...
      <div className="min-h-screen bg-[#0a0a0a] text-white lg:flex">
        <aside className="lg:sticky lg:top-0 lg:h-screen lg:w-64 border-r border-[#1f1f1f] bg-[#111] flex flex-col">
          <div className="h-16 px-5 flex items-center gap-3 border-b border-white/5">
            ...
            <div>
              <div className="font-bold leading-none">WA Auto Reply</div>
              <div className="text-[10px] uppercase tracking-[0.2em] text-gray-500 mt-1">AI Assistant</div>
            </div>
          </div>

          <nav className="p-3 flex-1 overflow-y-auto">
            <div className="text-xs text-gray-500 px-3 mb-2 mt-2">MODULES</div>
          ...
          <div className="px-4 py-3 border-t border-white/5">
            <div className="flex items-center gap-3">
              ...
              <button onClick={() => setShowLogoutConfirm(true)} className="text-gray-400 hover:text-red-400 ml-2" aria-label="Logout">
                <LogOut size={16} />
              </button>
            </div>
          </div>
          ...
          <header className="sticky top-0 z-40 h-16 border-b border-white/5 bg-[#0a0a0a]/90 backdrop-blur px-4 md:px-8 flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-3 md:gap-4">
              ...
              <div className="flex items-center gap-2">
                ...
                {waStatus === 'connected' ? (
                  <button onClick={handleDisconnect} className="hidden sm:flex items-center gap-2 text-xs border border-white/10 hover:border-red-400/40 hover:text-red-400 px-3 py-2 rounded-xl transition-colors">
                    <WifiOff size={14} /> Disconnect
                  </button>
                ) : (
                  <button onClick={() => handleConnect()} className="flex items-center gap-2 text-xs bg-[#25D366] hover:bg-[#1ebe5d] text-black font-semibold px-3 py-2 rounded-xl transition-colors">
          ...
          <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
            <div className="bg-[#111] border border-white/5 rounded-2xl p-8 text-center max-w-sm w-full">
              <div className="flex items-start justify-between gap-4 mb-4">
                ...
                <button onClick={() => setShowQR(false)} className="text-gray-500 hover:text-white transition-colors" aria-label="Close QR popup">
                  <X size={18} />
                </button>
              </div>
              <div className="bg-[#0a0a0a] border border-white/5 rounded-xl p-5 mb-5 min-h-[17rem] flex flex-col items-center justify-center">
                {qrImage ? (
                  <div className="bg-white p-4 rounded-xl inline-block mb-4">
                    <Image src={qrImage} alt="QR Code" width={192} height={192} unoptimized className="w-48 h-48" />
                  </div>
                ) : (
              ...
              <div className="flex items-center gap-3">
                <button onClick={handleRegenerateQr} className="flex-1 bg-[#25D366] hover:bg-[#1ebe5d] text-black font-semibold px-4 py-2.5 rounded-xl transition-colors text-sm">
                  Re-generate QR
                </button>
                <button onClick={() => setShowQR(false)} className="flex-1 border border-white/10 hover:border-white/20 text-white font-semibold px-4 py-2.5 rounded-xl transition-colors text-sm">
                  Close
                </button>
              </div>
