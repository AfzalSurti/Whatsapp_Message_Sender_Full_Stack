...
export default function AITemplatesPage() {
  ...
  const loadExampleTemplate = async (slug = "welcome") => {
    try {
      ...
      setForm({
        ...
        exampleConversations: example.exampleConversations || [
          emptyConversation(),
        ],
        sharedDocuments: [],
      });
  ...
  const handleDelete = async (template) => {
    try {
      await aiTemplateAPI.deleteTemplate(template);
    ...
    try {
      ...
      setForm((prev) => {
        ...
        next[conversationIndex] = {
          ...next[conversationIndex],
          mediaFiles: [
            ...(next[conversationIndex].mediaFiles || []),
            ...uploaded,
          ],
        };
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white p-6 md:p-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <p className="text-gray-400 mt-2">
              Teach the bot with real user/bot examples, business details, and
              files to share
            </p>
          </div>
          {activeTab === "templates" && (
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {starterTemplates.map((starter) => {
                    const alreadyAdded = templates.some(
                      (t) => t.name === starter.name,
                    );
                      ...
                      >
                        ...
                        <div className="flex gap-2">
                          <button
                            ...
                            disabled={
                              alreadyAdded || addingStarter === starter.slug
                            }
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                {templates.length === 0 ? (
                  <div className="col-span-full border border-dashed border-white/10 rounded-2xl p-12 text-center text-gray-400">
                    No templates yet. Add the{" "}
                    <strong>Internship Inquiry</strong> starter above or click{" "}
                  ...
                  templates.map((template) => (
                    <div
                      key={template._id}
                      className="border border-white/10 rounded-2xl p-5 bg-[#111] space-y-3"
                    >
                ...
                {leads.length === 0 ? (
                  ...
                  leads.map((lead) => (
                    <tr key={lead._id} className="border-t border-white/10">
                      ...
                      <td className="px-4 py-4">
                        {Object.entries(lead.collectedInfo || {}).map(
                          ([k, v]) => (
                            <span
                              key={k}
                              className="inline-block mr-2 mb-1 px-2 py-1 rounded-full bg-white/5 text-xs"
                            >
                              {k}: {String(v)}
                            </span>
                          ),
              ...
              <div className="p-5 space-y-8 max-h-[75vh] overflow-y-auto">
                ...
                <section className="space-y-3">
                  ...
                  <p className="text-xs text-gray-500">
                    Use {"{{field_key}}"} in bot replies. Example:{" "}
                    {"{{business_name}}"}
                  </p>
                ...
                <section className="space-y-4">
                  ...
                  <p className="text-xs text-gray-500">
                    Show AI how user messages and bot replies should look.
                    Example: User says &quot;Hi&quot; → Bot welcomes them.
                  </p>
                          ...
                          <button
                            ...
                            onClick={() =>
                              setForm({
                                ...form,
                                exampleConversations:
                                  form.exampleConversations.filter(
                                    (_, i) => i !== index,
                                  ),
                              })
                            }
                ...
                <section className="space-y-3">
                  ...
                  <textarea
                    value={form.aiAdvice}
                    onChange={(e) =>
                      setForm({ ...form, aiAdvice: e.target.value })
                    }
                ...
                <section className="space-y-4">
                  ...
                  <p className="text-xs text-gray-500">
                    When user message contains a keyword (brochure, fees,
                    syllabus), AI sends that file.
                  </p>
                  {form.sharedDocuments.map((doc, index) => (
                    <div
                      key={`doc-${index}`}
                      className="border border-white/10 rounded-xl p-4 bg-[#0a0a0a] space-y-2"
                    >
                      <input
                        value={doc.name}
                        onChange={(e) => {
          ...
          <div className="w-full max-w-3xl bg-[#111] border border-white/10 rounded-2xl max-h-[90vh] overflow-hidden flex flex-col">
            ...
            {detailLoading ? (
              ...
              <div className="p-5 overflow-y-auto space-y-3">
                {(conversationDetail?.conversationHistory || []).map(
                  (entry, index) => (
                    <div
                      key={index}
      ...
      <ConfirmModal
        ...
        message="Your AI Template will be deleted and will loss all the template Data."
